...
import gg.vape.event.listener.VaporClientEventListener;
import gg.vape.event.listener.VaporLifecycleEventListener;
import gg.vape.event.listener.VaporShutdownEventListener;
...
public class Vapor {
    ...
    public void loadConfigData(JsonObject configRoot, boolean useNewOtherDataKey) {
        ...
        if ((profilesElement = ConfigJsonUtils.getJsonObject(configRoot, "profiles")) != null) {
            ...
            Vapor.debugLog("profilesData is NULL!");
        }
        ...
        if (otherData != null && otherData.size() > 0) {
            ...
            Vapor.debugLog("otherData is NULL!");
        }
    }
    ...
    public void loadMappings() {
        this.traceStep(18);
        int opaqueSeed = Vapor.getOpaqueState();
        MappedClasses.p();
        mappingsLoaded = true;
        ...
        if (opaqueBranch != 0) {
            ...
            if (GuiComponent.getLegacyComponentState() == null) {
                Vapor.setOpaqueState(++opaqueBranch);
            }
            return;
        }
        this.traceStep(19);
    ...
    private void logInitError(String phase, Throwable error) {
        ...
        message.append(Vapor.formatThrowable(error));
        Vapor.logError(message.toString());
    ...
    public boolean initAccountInfo() {
        ...
        try {
            ...
            if (response == null || !response.isSuccessful() || response.getData() == null) {
                ...
                Vapor.logError("Failed to initialize account details1: " + error);
                return false;
            }
            AccountInfo initializedAccount = AccountInfo.fromResponse(response.getData());
            if (initializedAccount == null) {
                Vapor.logError("Failed to initialize account details1: empty account data");
                return false;
            }
            this.accountInfo = initializedAccount;
        }
        catch (CancellationException | CompletionException error) {
            Vapor.logError("Failed to initialize account details2: " + Vapor.formatThrowable(error));
            return false;
        }
        return true;
    }
    ...
    public void initializeManagers() {
        ...
        if (NativeBridge.isFabricLoaderPresent()) {
            NativeBridge.sce("Fabric: skipping legacy item-stack mapping warmup on Minecraft 26.2");
        } else {
        ...
        // Online services are intentionally disabled in the Fabric conversion.
    }
    ...
    public static int opaquePredicate() {
        int state = Vapor.getOpaqueState();
        if (state == 0) {
            return 20;
        }
        return 0;
    ...
    public boolean isLabyModPresent() {
        ...
        try {
            Class.forName("net.laby.launcher.classloading.LabyClassLoader", false, Vapor.class.getClassLoader());
    ...
    private void registerEventListeners() {
        ...
        EventBus.getInstance().registerListener(AttackStrengthTracker.INSTANCE, Vapor::lambda$registerListeners$0);
        ...
        EventBus.getInstance().registerListener(EventTimingOverlayListener.INSTANCE, Vapor::lambda$registerListeners$1);
        EventBus.getInstance().registerListener(new VaporShutdownEventListener(), new Predicate[0]);
        ...
        if (ForgeVersion.MC_1_8_9.L()) {
            EventBus.getInstance().registerListener(new VaporLifecycleEventListener(), new Predicate[0]);
        }
        if (ForgeVersion.MC_1_21_10.d()) {
            EventBus.getInstance().registerListener(new VaporClientEventListener(), new Predicate[0]);
        }
    }

    private void showLoadCompleteNotification() {
        // Fabric uses VaporFabricMenuScreen, not the legacy font-backed overlay.
        if (NativeBridge.isFabricLoaderPresent()) {
            return;
        }
    ...
    public void initializeRender() {
        ...
        try {
            ...
            try {
                ...
                this.logInitError("VaporRender initialization", error);
                return;
            }
    ...
    public static void logThrowable(Throwable error) {
        Vapor.debugLog(Vapor.formatThrowable(error));
    }
    ...
    private void initPrimaryMappingTasks() {
        ...
        int opaqueSeed = Vapor.opaquePredicate();
        this.primaryMappingTaskSet = new PrimaryMappingTaskSet();
        ...
        try {
            while (!ClientSettings.framesInitialized) {
                try {
                    ...
                    Vapor.logThrowable(interrupted);
            ...
            try {
                throw Vapor.rethrow(completionException);
            }
            catch (CompletionException retriedCompletionException) {
                throw Vapor.rethrow(retriedCompletionException);
            }
        }
    ...
    public static void debugLog(String message) {
        ...
        try {
            ...
            System.err.println("[Vapor421] " + normalizedMessage);
        }
    }
    ...
    public Vapor() {
        int opaqueBranch = Vapor.opaquePredicate();
        if (opaqueBranch != 0) {
